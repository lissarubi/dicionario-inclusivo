        ...
        dictionary.forEach { dictionaryEntry ->
            ...
            keys.forEach { wrongWord ->
                if (textBeingTranslated[index] == wrongWord) {
                    ...
                    textBeingTranslated[index] = fixWrongWord(wrongWord, correctWord = dictionaryEntry.word.values.first(), word)

                    if (index != 0){
                        textBeingTranslated[index - 1] = fixWordGender(textBeingTranslated[index - 1], dictionaryEntry.wordGender, plural)
                    }
                }
            }
        }
